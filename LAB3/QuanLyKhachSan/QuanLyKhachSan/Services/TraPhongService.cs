...
namespace QuanLyKhachSan.Services
{
    ...
    public class TraPhongService
    {
        ...
        {
            return Db.Query(@"SELECT h.SoHoaDon, h.SoPhieuDat, h.TienPhong, h.TienDichVu, h.TongTien, h.TrangThai, h.NgayLap, k.HoTen 
                              FROM HoaDon h 
        ...
        public KetQuaXuLy LapPhieuDenBu(string soDB, string soDat, string phong, DateTime ngay, string maNV, List<DenBuItem> ds)
        {
            if (string.IsNullOrWhiteSpace(soDB) || string.IsNullOrWhiteSpace(soDat) || 
                ...
                return KetQuaXuLy.Fail("Phiếu đền bù chưa đủ thông tin hoặc chưa chọn tiện nghi đền bù.");
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    var h = new SqlCommand(@"INSERT INTO PhieuDenBu(SoPhieuDenBu, SoPhieuDat, SoPhong, NgayLap, MaNV, TongTien) 
                                             VALUES(@so, @d, @p, @n, @nv, @t)", cn, tx);
                    ...
                    foreach (var x in ds)
                    {
                        var c = new SqlCommand(@"INSERT INTO ChiTietPhieuDenBu(SoPhieuDenBu, MaTienNghi, MucDoThietHai, SoTien) 
                                                 VALUES(@so, @tn, @m, @t)", cn, tx);
        ...
        public KetQuaXuLy LapHoaDon(string soHD, string soDat, DateTime ngay, string maNV, int soNgay)
        {
            ...
            try
            {
                ...
                return KetQuaXuLy.Ok("Đã lập hóa đơn tiền phòng và dịch vụ thành công.");
            }
            catch (Exception ex)
            {
                return KetQuaXuLy.Fail("Lỗi: " + ex.Message);
        ...
        public KetQuaXuLy ThanhToan(string maTT, string soHD, DateTime ngay, string hinhThuc, decimal tien)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    var i = new SqlCommand(@"INSERT INTO ThanhToan(MaThanhToan, SoHoaDon, NgayThanhToan, HinhThuc, SoTien) 
                                             VALUES(@m, @h, @n, @ht, @t)", cn, tx);
        ...
        public KetQuaXuLy TraPhong(string soDat, DateTime ngayTra)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    var u2 = new SqlCommand(@"UPDATE Phong SET TrangThai = N'Trống' 
                                             WHERE SoPhong IN (SELECT SoPhong FROM ChiTietDatPhong WHERE SoPhieuDat = @s)", cn, tx);
