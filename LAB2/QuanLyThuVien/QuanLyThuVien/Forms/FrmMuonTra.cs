...
namespace QuanLyThuVien.Forms
{
    public partial class FrmMuonTra : Form
    {
        ...
        private void FrmMuonTra_Load(object sender, EventArgs e)
        {
            ...
            cboTinhTrang.SelectedIndexChanged += (s, ev) => CapNhatGoiYPhiPhat();
            dtNgayTra.ValueChanged += (s, ev) => CapNhatGoiYPhiPhat();
            dgvDangMuon.SelectionChanged += (s, ev) => CapNhatGoiYPhiPhat();
        }
        ...
        private string MaDocGiaMuon => cboDocGia.SelectedValue == null ? "" : cboDocGia.SelectedValue.ToString();
        private string MaDocGiaTra => cboDocGiaTra.SelectedValue == null ? "" : cboDocGiaTra.SelectedValue.ToString();
        private string MaNhanVienMuon => cboNhanVienMuon.SelectedValue == null ? "" : cboNhanVienMuon.SelectedValue.ToString();
        private string MaNhanVienTra => cboNhanVienTra.SelectedValue == null ? "" : cboNhanVienTra.SelectedValue.ToString();
        ...
        private void btnThemSach_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Chỉ được chọn tối đa 3 đầu sách khác nhau.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            foreach (DataRow row in selectedBooks.Rows)
            {
                ...
                {
                    MessageBox.Show("Đầu sách này đã có trong danh sách mượn.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
            }
        ...
        private void btnBoSach_Click(object sender, EventArgs e)
        {
            ...
            {
                DataRowView r = dgvSachChon.CurrentRow.DataBoundItem as DataRowView;
                if (r != null) r.Row.Delete();
            }
        }
        ...
        private void btnTaiSachMuon_Click(object sender, EventArgs e)
        {
            TaiSachDangMuon();
        }
        ...
        {
            if (dgvDangMuon.CurrentRow == null || dgvDangMuon.CurrentRow.DataBoundItem == null)
            {
        ...
        private void CapNhatGoiYPhiPhat()
        {
            if (dgvDangMuon.CurrentRow == null || dgvDangMuon.CurrentRow.DataBoundItem == null) return;
            DataRowView r = dgvDangMuon.CurrentRow.DataBoundItem as DataRowView;
            if (r == null) return;
            ...
            {
                DateTime henTra = Convert.ToDateTime(r["NgayHenTra"]).Date;
                DateTime tra = dtNgayTra.Value.Date;
                if (tra > henTra)
                {
                    int soNgayTre = (tra - henTra).Days;
                    phi += soNgayTre * 5000;
                }
            }

            string tt = Convert.ToString(cboTinhTrang.SelectedItem) ?? "";
            if (tt.IndexOf("Rách", StringComparison.OrdinalIgnoreCase) >= 0 || tt.IndexOf("Hư hỏng", StringComparison.OrdinalIgnoreCase) >= 0)
                phi += 20000;
            else if (tt.IndexOf("Mất", StringComparison.OrdinalIgnoreCase) >= 0)
                phi += 50000;

            numPhiPhat.Value = phi;
        }
